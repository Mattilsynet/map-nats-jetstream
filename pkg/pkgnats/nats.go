...
func CreateNatsConnection(clientName, credentialsFileContent, natsUrl string) (*nats.Conn, error) {
	...
	if err != nil {
		slog.Info("pkgnats: Error creating nats connection: " + err.Error())
		return nil, err
	}
	tmpCredsFile.Close()
...
func setupNatsConnectionOpts(opts []nats.Option) []nats.Option {
	...
	opts = append(opts, nats.DisconnectErrHandler(func(nc *nats.Conn, err error) {
		slog.Info("Nats disconnected", "nats", "disconnected")
		panic("nats disconnect, exiting application such that we don't force application to have nats creds in memory")
	}))
